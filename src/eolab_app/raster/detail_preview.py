"""Fixed-policy bounded numeric images for overview-limited raster detail."""
...
def _limits(
    ...
    Args:
        rendering: Sampled or exact-window result.
...
def read_raster_detail_preview(
    ...
    """Read the fixed bounded preview from an overview-limited raster.

    Broad views use 127 center samples on the projected rectangle's longest
    edge and preserve its aspect ratio on the shorter edge.
    ...
    fixed limits; broader views retain the fixed center-sampling policy.
    ...
    admitted native block is read once without ``out_shape``. Sampled grids are
    already aligned to the Web Mercator target rectangle.

...
class MountedVectorResolver:
    ...
    def resolve(self, item: dict[str, Any]) -> ResolvedVectorSource:
        ...
        Raises:
            RasterAssetError: If source, format, Asset, or layer identity is
                missing, contradictory, or outside the scan mount.
            RasterConflictError: If a formerly mounted source is unavailable.
        """
        if item.get("collection") != MOUNTED_VECTOR_COLLECTION_ID:
            raise RasterAssetError("The Item is not a mounted vector dataset")
        ...
        if asset_uri.scheme != "file":
            if declared_kind == "mounted":
                raise RasterAssetError(
                    "The mounted vector source contract points to a remote Asset"
                )
    ...
    def _source_contract(
        ...
        Raises:
            RasterAssetError: If the explicit or legacy identity is invalid.
        """
        ...
        if metadata is not None:
            if not isinstance(metadata, dict) or set(metadata) != {
                ...
                raise RasterAssetError("The vector source contract is invalid")
            ...
            if metadata.get("asset_key") != expected_asset_key:
                raise RasterAssetError("The vector source Asset identity conflicts")
            layer_name = metadata.get("layer_name")
            if layer_name is not None and (
                not isinstance(layer_name, str) or not layer_name
            ):
                raise RasterAssetError("The vector layer identity is invalid")
            return (
                source_format,
                expected_asset_key,
                layer_name,
        ...
        if not isinstance(item_id, str):
            raise RasterAssetError("The vector Item identity is invalid")
        matching_formats = [
            source_format
        ...
        if len(matching_formats) != 1:
            raise RasterAssetError("The vector source format is not explicit")
    ...
    def _legacy_layer_name(
        ...
        Raises:
            RasterAssetError: If a multi-layer identity is absent or conflicts.
        """
        properties = item.get("properties")
        assets = item.get("assets")
        asset = assets.get(asset_key) if isinstance(assets, dict) else None
        if not isinstance(properties, dict) or not isinstance(asset, dict):
            raise RasterAssetError("The vector source identity is incomplete")
        if source_format == "geopackage":
            ...
            if (
                ...
                raise RasterAssetError("The GeoPackage layer identity is invalid")
        ...
        if source_format == "shapefile":
            ...
            if not isinstance(href, str):
                raise RasterAssetError("The Shapefile layer identity is invalid")
    ...
    def _resolve_mounted_uri(self, asset_href: str) -> Path:
        ...
        Raises:
            RasterAssetError: If the URI escapes the mount.
            RasterConflictError: If the path no longer exists.
        """
    ...
    def _validate_source_path(
        ...
        Raises:
            RasterAssetError: If file/directory shape or suffix conflicts.
        """
        suffixes = {
            "shapefile": ".shp",
            "geopackage": ".gpkg",
    ...
    def _resolve_shapefile_components(
        ...
        Raises:
            RasterAssetError: If required components are absent, ambiguous, or
                do not share the primary file's exact directory and stem.
        """
        assets = item.get("assets")
        if not isinstance(assets, dict):
            raise RasterAssetError("The Shapefile component Assets are missing")
        paths: dict[str, Path] = {}
        for extension in SHAPEFILE_COMPONENT_TYPES:
            ...
            if (
                ...
                raise RasterAssetError("A Shapefile component Asset is invalid")
...
class PublishedVectorRegistry:
    ...
    def authorize(
        ...
        Raises:
            RasterConflictError: If the mounted source changed or disappeared.
        """
        try:
    ...
    def require_current(self, layer_name: str) -> AuthorizedVector:
        ...
        Raises:
            RasterRequestError: If this process did not authorize the layer.
            RasterConflictError: If any source component changed afterward.
        """
        ...
        return AuthorizedVector(source, approved_signature, style_name)

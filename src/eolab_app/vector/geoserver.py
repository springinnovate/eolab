...
class GeoServerVectorPublisher(GeoServerRasterPublisher):
    """Converge exact mounted vector layers using the raster recovery boundary.

    Transport handling, exact accepted-status enforcement, response
    sanitization, and stable failure categories are inherited from the #119
    raster adapter. This class supplies only vector resource topology and
    transitions.
    """
    ...
    async def publish(
        ...
        Raises:
            RasterPublicationError: If GeoServer is unavailable, initialized
    ...
    async def _inspect_vector_state(
        ...
        Raises:
            RasterPublicationError: If inspection violates its response contract.
        """
        ...
        datastore_exists = await self._resource_exists(
            "inspect vector datastore",
            f"{self._datastore_url(resource_name)}.json?quietOnNotFound=true",
        )
        feature_type_exists = await self._resource_exists(
    ...
    async def _create_vector_publication(
        ...
        Raises:
            RasterPublicationError: If creation or verification fails.
        """
        try:
            await self._request(
    ...
    async def _require_native_layer_identity(
        ...
        Raises:
            RasterPublicationError: If GeoServer returns invalid metadata or a
    ...
    async def _rollback_new_vector_publication(self, resource_name: str) -> None:
        ...
        """
        try:
            exists = await self._resource_exists(
    ...
    async def _delete_datastore(
        ...
        Raises:
            RasterPublicationError: If deletion does not return exactly 200.
        """
        await self._request(
            operation,
            "DELETE",
    ...
    async def _assign_vector_style(
        ...
        Raises:
            RasterPublicationError: If style assignment is rejected.
        """
        await self._request(
            "assign vector style",
            "PUT",
            f"{self._layer_url(resource_name)}.json",
            accepted_statuses=frozenset({200}),
            json={
                "layer": {
                    "defaultStyle": {
    ...
    def _require_vector_prerequisites(
        ...
        Raises:
            RasterPublicationError: If either prerequisite is absent.
        """

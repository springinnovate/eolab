...
standard STAC Collection and Item identity, repeats the current assessment and
...
without exporting that metadata. The current Catalog Item and fresh
...
windows, temporary AOIs, server status, pending work, and low-resolution detail
previews. URL-fragment sharing can be added later as a transport for the same

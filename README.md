...
5. Configure the read-only scan mount and the directories EOLab should search. In the EOLab resource's **Production** environment variables, select **Add** and create these variables:
    ...
    EOLAB_SCAN_REMOTE_S3_SOURCES=[]
    ```

    `EOLAB_SCAN_MOUNT_PATH` is an absolute path on the deployment server. Coolify does not list it automatically because Compose uses it as a bind-mount source, so add it manually. Each entry in `EOLAB_SCAN_PATHS_WITHIN_MOUNT` is relative to that mount; use `["."]` to scan the entire mount. `EOLAB_SCAN_DISPLAY_PATH_PREFIX` is the path description shown to users in the Item inspector. `EOLAB_SCAN_REMOTE_S3_SOURCES` optionally adds S3-compatible bucket/prefix roots; configure its credentials and endpoint as Coolify secrets using the variables below. `EOLAB_SCAN_WORKER_COUNT` is the number of concurrent mounted metadata processes. `EOLAB_SCAN_WRITER_COUNT` is the maximum number of catalog bulk upserts in progress at once. `EOLAB_SCAN_BATCH_SIZE` is the maximum number of Items in each bulk upsert. Keep the defaults initially; the additional scanner limits in the runtime configuration table are intended for measured tuning. The mounted directories and dataset files must be readable by the application container.
...
| `EOLAB_SCAN_REMOTE_S3_SOURCES`      | `[]`                                              | JSON array of S3 bucket/prefix source descriptions  |
| `EOLAB_S3_ENDPOINT_URL`             | none                                              | Optional private S3-compatible service origin       |
| `EOLAB_S3_REGION`                   | `us-east-1`                                       | S3 signing region                                   |
| `EOLAB_S3_ACCESS_KEY_ID`            | provider chain                                    | Optional read-only access-key secret                |
| `EOLAB_S3_SECRET_ACCESS_KEY`        | provider chain                                    | Optional read-only secret-key secret                |
| `EOLAB_S3_SESSION_TOKEN`            | none                                              | Optional temporary credential secret                |
| `EOLAB_S3_LIST_PAGE_SIZE`           | `500`                                             | Objects requested per bounded listing page          |
| `EOLAB_S3_METADATA_CONCURRENCY`     | `4`                                               | Concurrent remote metadata reads                    |
...
## Scan configured datasets

Open the **Catalog: connected** System state menu and select **Scan sources**. The menu lists each configured mounted or remote location using its user-facing description and reports either live scan progress or the most recent completion time since EOLab started. EOLab searches each configured mounted path recursively for GeoTIFF (`.tif` and `.tiff`), GeoPackage (`.gpkg`), mounted ESRI Shapefile (`.shp`), ZIP-contained Shapefile (`.zip`), GeoJSON FeatureCollection (`.geojson`), and Esri File Geodatabase (`.gdb` directory) datasets, matching extensions case-insensitively. Configured S3-compatible prefixes initially support remote GeoTIFF and Shapefile datasets. Generic `.json` files are not scanned. Live status reports source datasets discovered and processed separately from catalog Items produced and written, then classifies written Items as new or already present. Error details are collapsed by default and remain independently scrollable when opened, so the Catalog results remain usable during a scan. EOLab reads mounted metadata using `EOLAB_SCAN_WORKER_COUNT` concurrent processes, remote metadata using `EOLAB_S3_METADATA_CONCURRENCY` bounded workers, and uses `EOLAB_SCAN_WRITER_COUNT` concurrent STAC Bulk Transactions, each containing at most `EOLAB_SCAN_BATCH_SIZE` Items from one Collection. Catalog-write timing is cumulative across writers and can exceed wall time. A failure in one dataset does not stop the remaining scan; a catalog inventory, listing, or write failure stops the scan.
...
Scans create or update the legacy-ID `eolab-mounted-geotiffs` and `eolab-mounted-vectors` STAC Collections for both mounted and remote sources. Mounted Item identifiers remain derived from the primary path relative to the mount. Remote Item identifiers use the configured source ID, bucket, and exact primary key; Assets record the observed ETag or provider version separately. Cleanup verifies all required mounted and remote Assets before deleting anything and stops without deletion if either source kind cannot be verified. STAC Assets retain `file:` or unsigned `s3:` URIs and explicit `eolab:source_kind` metadata. Remote Assets are catalog-only and are not accepted by mounted rendering, styling, sampling, or processing paths.

The complete S3 configuration, paging, range-read, change-during-scan, restart, identity, and deleted/moved-object policies are documented in [Remote object-storage scan contract](docs/remote-object-storage.md).

...
Large rasters rejected only because internal overviews or the coarsest overview scale are inadequate can instead offer an explicit **detail-only visualization** after the current deployed reader accepts their CRS. The user chooses a center-pixel sample, a fixed small sampling grid, or a deterministic representative bounded patch. EOLab produces these approximate previews with strictly bounded Rasterio/GDAL source-window reads, displays them at their spatial positions with the raster extent, and does not publish the raster or send arbitrary full-extent WMS requests. Nodata stays nodata, and the raster extent is not presented as a valid-data footprint. See [Raster detail-only visualization](docs/raster-detail-only-preview.md) for applicability, exact resource bounds, cache identity, and approximation semantics.

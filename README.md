...
Copy `.env.example` to `.env`, set `EOLAB_DATABASE_PASSWORD`, and choose `true` or `false` for `EOLAB_LOAD_SAMPLE_CATALOG`. To display a Git-derived version locally, replace the `SOURCE_COMMIT` fallback with the full 40-character SHA reported by `git rev-parse HEAD`.

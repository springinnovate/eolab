...
6. Configure the read-only scan mount and the directories EOLab should search. In the EOLab resource's **Production** environment variables, select **Add** and create these variables:
    ...
    Mount the source filesystem read-only on the deployment server itself—for example, use the `ro` option in `/etc/fstab`. Compose also requests a read-only bind for both EOLab and GeoServer. Those are the primary protections. As a backup verification, both container entrypoints inspect `/proc/self/mountinfo` and refuse to start unless the kernel reports `/scan-source` as effectively read-only. This extra check exists because Coolify or Docker inspection metadata may describe the bind as `rw` even when an underlying read-only NFS mount still makes the container's effective mount `ro`; it does not replace either the host-level or Compose-level protection.
...
If GeoServer exhausts its Java heap, the JVM exits instead of remaining unresponsive. Docker's `unless-stopped` policy then restarts the GeoServer container; the rendering status may be unavailable while it starts, but the EOLab catalog remains available.

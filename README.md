...
Scans create or update the `eolab-mounted-geotiffs` and `eolab-mounted-vectors` STAC Collections. Item identifiers are derived from each primary file's path relative to the mounted root, so scanning the same path again updates its Item instead of adding a duplicate. In parallel with metadata extraction, catalog cleanup checks the stored source Assets for every scanner-owned Item and removes Items whose required files no longer exist. This follows the stored Asset paths across the whole mount, so omitting a directory from `EOLAB_SCAN_PATHS_WITHIN_MOUNT` does not imply deletion. The checks add one metadata lookup per GeoTIFF and four per Shapefile; they are limited to eight concurrent filesystem operations and reported separately in scan status and timing.

Cleanup is deliberately all-or-nothing. EOLab completes the full read-only verification pass and confirms that the mounted root remained the same before issuing any bounded database delete. `FileNotFoundError` marks one Item as missing; a permissions error, NFS error, malformed scanner Asset, changed mount, or database error fails cleanup and deletes nothing. Metadata indexing can still complete when cleanup fails, and the cleanup failure is shown separately from dataset errors. STAC Assets retain their container `file:` URIs; the inspector combines their mount-relative titles with `EOLAB_SCAN_DISPLAY_PATH_PREFIX` so users see each location as it is known on their own system.

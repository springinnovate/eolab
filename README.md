...
5. Create the directory that EOLab will scan on the Coolify deployment server. In Coolify's global **Production** environment variables, set `EOLAB_SCAN_SOURCE_PATH` to that absolute host path, for example `/mnt/eolab/geotiffs`. Docker Compose mounts it read-only at `/scan-source` in the application container. The path must exist on the deployment server and its directories and GeoTIFFs must be readable by the application container; a path on your workstation is not visible to a remote Coolify server.

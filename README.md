...
Mounted Shapefile and GeoPackage layers can be added to EOMap after their exact source layer passes the deployed GeoServer reader assessment. Catalog vectors render as bounded, one-layer WMS overlays with distinct fixed point, line, and polygon symbology and share fitting, visibility, ordering, opacity, and removal with raster layers. GeoJSON, ZIP-contained Shapefile, File Geodatabase, and remote vector Assets remain catalogable but show a precise capability reason instead of being reinterpreted through an unsafe or ambiguous publication path. The source, recovery, browser, and new-format contracts are documented in [Catalog vector publication contract](docs/vector-publication.md).

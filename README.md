...
Large rasters rejected only because internal overviews or the coarsest overview scale are inadequate can instead offer an explicit **bounded sampled raster** after the current deployed reader accepts their CRS. EOLab builds a low-resolution full-extent proxy by sampling each large source region at its center or at a fixed representative set of positions; a deterministic bounded detail patch remains available as the local alternative. Native blocks are read once under fixed block-count and decoded-byte ceilings, then only the bounded in-memory values are warped and colored with the normal raster ramp. The raster is never published and no arbitrary full-extent WMS request is made. Nodata stays nodata, and the raster extent is not presented as a valid-data footprint. See [Bounded sampled-raster visualization](docs/raster-detail-only-preview.md) for applicability, exact resource bounds, cache identity, and approximation semantics.

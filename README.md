...
One Shapefile Item groups files with the same exact base name. The `.shp`, `.shx`, `.dbf`, and `.prj` components are required; recognized `.cpg`, `.qix`, `.sbn`, `.sbx`, and `.shp.xml` companions are included when present. Missing or unreadable required components produce one dataset error. The Item records the native CRS and bounds, feature count, declared layer geometry type, and field names and types using published STAC Projection and Table extensions. Shapefile drivers describe multipart line and polygon records under their declared `LineString` or `Polygon` layer type; EOLab does not iterate every feature merely to distinguish multipart records during cataloging. Empty Shapefiles are cataloged without a footprint or bounding box.

...
def test_scan_finalizer_preserves_remote_raster_capability_boundary(
    tmp_path: Path,
) -> None:
    """Leave a remote raster catalog-only without resolving a mounted path.

    Args:
        tmp_path: Temporary path that must remain unused by the finalizer.
    """
    item = {
        ...
        "assets": {
            "data": {
                "href": "s3://example-bucket/imagery/scene.tif",
                "roles": ["data"],
            }
        },
    }
    ...
    finalizer = RasterAssessmentFinalizer(
        _Resolver(tmp_path / "must-not-be-resolved.tif"),
        reader_assessor,
    )

    assert asyncio.run(finalizer.finalize(item)) is item
    assert "eolab:rendering" not in item["assets"]["data"]
    assert reader_assessor.paths == []
